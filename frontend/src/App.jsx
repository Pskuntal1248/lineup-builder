...
const aspectRatioOptions = [
  { key: 'square', ratio: '1:1', icon: '◼' },
  { key: 'portrait', ratio: '4:5', icon: '▮' }
]
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-0.5 p-0.5 bg-[#1a1a1f] rounded-lg">
                {aspectRatioOptions.map((option) => (
                  <button
                    key={option.key}
                    onClick={() => updateSettings({ aspectRatio: option.key })}
                    className={`
                      flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-medium transition-all duration-150
                      ${settings.aspectRatio === option.key 
                        ? 'bg-emerald-500 text-white' 
                        : 'text-zinc-400 hover:text-white'
                      }
                    `}
                  >
                    <span className="text-[10px] opacity-80">{option.icon}</span>
